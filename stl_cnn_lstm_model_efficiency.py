...
Added computational efficiency testing metrics.
...
def analyze_class_distribution(y_stl, split_name="Dataset"):
    class_names = [
        "Normal [0,0,0]", "Missing [1,0,0]", "Trend [0,1,0]", "Drift [0,0,1]",
        "Missing+Trend [1,1,0]", "Missing+Drift [1,0,1]", "Trend+Drift [0,1,1]"
    ]
...
def main(x_train_fold, y_train_fold, x_val_fold, y_val_fold, x_test, y_test,
    ...
    return model, {}, mtl_results, y_pred_stl, y_true_stl_test
...
if __name__ == "__main__":
    ...
    for run_id in range(1, num_runs + 1):
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(x_train_val, stratify_labels), 1):
            if fold == 4:
                ...
                model, _, mtl_results, y_pred_stl, y_true_stl_test = main(
                ...
                for task in ['1_missing', '2_trend', '3_drift']:
                    ...
                    auc_val = mtl_results[task]['roc_auc']
                    report_logs.append(f"\nTask: {task}\nROC AUC: {auc_val:.4f}\n")
                    print(f"Task: {task} | ROC AUC: {auc_val:.4f}")
            else:
                continue
    ...
    efficiency_report = [
        "\n" + "="*60, "COMPUTATIONAL EFFICIENCY ANALYSIS", "="*60,
        f"\nReport Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"Device: {torch.device('cuda' if torch.cuda.is_available() else 'cpu')}"
    ]
    if torch.cuda.is_available(): efficiency_report.append(f"GPU: {torch.cuda.get_device_name(0)}")
    
    efficiency_report.extend([
        f"\nTotal Training Runs: {num_runs}",
        f"Total Folds per Run: 1 (only fold 4)",
        f"Total Epochs per Run: 100\n",
        "-"*60, "PER-RUN METRICS", "-"*60
    ])
    
    all_epoch_times, all_gpu_memory, all_cpu_memory, all_throughputs, all_run_times = [], [], [], [], []
    
    for run_summary in all_runs_metrics:
        efficiency_report.extend([
            f"\nRun {int(run_summary['run_id'])}:",
            f"  Total Run Time: {run_summary['total_run_time']:.2f}s",
            f"  Average Epoch Time: {run_summary['avg_epoch_time']:.4f}s",
            f"  Throughput: {run_summary['throughput_samples_per_sec']:.2f} samples/sec",
            f"  Average GPU Memory: {run_summary['avg_gpu_memory_mb']:.2f} MB",
            f"  Average CPU Memory: {run_summary['avg_cpu_memory_mb']:.2f} MB",
            f"  Average Forward Pass Time: {run_summary['avg_forward_time']:.6f}s",
            f"  Average Backward Pass Time: {run_summary['avg_backward_time']:.6f}s",
        ])
    ...
    if all_epoch_times: efficiency_report.append(f"\nAverage Epoch Time: {np.mean(all_epoch_times):.4f}s (±{np.std(all_epoch_times):.4f}s)")
    if all_gpu_memory: efficiency_report.append(f"Average GPU Memory Usage: {np.mean(all_gpu_memory):.2f}MB (±{np.std(all_gpu_memory):.2f}MB)")
    if all_cpu_memory: efficiency_report.append(f"Average CPU Memory Usage: {np.mean(all_cpu_memory):.2f}MB (±{np.std(all_cpu_memory):.2f}MB)")
    if all_throughputs: efficiency_report.append(f"Average Throughput: {np.mean(all_throughputs):.2f} samples/sec (±{np.std(all_throughputs):.2f})")
    if all_run_times:
