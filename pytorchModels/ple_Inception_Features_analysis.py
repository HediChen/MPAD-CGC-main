...
class InceptionExpert(nn.Module):
    ...
    def forward(self, x):
        ...
        # metric = x.detach().cpu().numpy()
        # plot_metric_heatmap_and_signals(metric,shape=[4,8], save_dir="discussion/figures/metric_vis")
        
        x = self.LTA(x)
        # metric = x.detach().cpu().numpy()
        # plot_metric_heatmap_and_signals(metric,shape=[4,8], save_dir="discussion/figures/metric_vis/LTA")
        x = self.encoder(x)
        metric = x.detach().cpu().numpy()
        plot_metric_heatmap_and_signals(metric,shape=[8,8], save_dir="discussion/figures/metric_vis/encoder")
        x = self.fc(x)
        return x
...
class FeatureAttention(nn.Module):
    ...
    def forward(self, x):
        # x: (B, 9)
        print("Statistical Features:{}".format(x.cpu().numpy()))
        plot_feature_weights_heatmap(weights =x.cpu().numpy(), color = 'Greys')
        scores = self.attn_layer(x)  # (B, 9)
        weights = self.softmax(scores)  # (B, 9)
        print("FeatureAttention Weights:{}".format(weights.cpu().numpy()))
        attended = x * weights  # Element-wise weighting
        print("Attended Features:{}".format(attended.cpu().numpy()))
        plot_feature_weights_heatmap(weights =attended.cpu().numpy(), color = 'Greys')
        return attended
...
class GateNetwork(nn.Module):
    def __init__(self, inputs_dim, output_dim, num_experts, shared_experts_cnn):
        ...
        self.task_experts_dnn = nn.ModuleList([
            nn.Sequential(
                nn.Flatten(),
                # nn.BatchNorm1d(inputs_dim),
                FeatureAttention(input_dim=inputs_dim),
    ...
    def forward(self, inputs):
        ...
        temp = torch.softmax(self.gates(inputs[:,:,:1008]), dim=-1)
        temp1 = torch.softmax(self.gates(inputs[:,:,:1008]), dim=-1).squeeze(1)
