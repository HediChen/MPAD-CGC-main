...
class MMoeWithAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for idx, name in enumerate(self.labels_dict):
            ...
            # Pass tower_input as query, key, and value to the attention layer
            tower_output = self.task_tower[idx][2](tower_input, tower_input, tower_input)  # MultiHeadAttention layer
            tower_output = self.task_tower[idx][3](tower_output)  # Linear layer after attention
            task_output = self.task_dense[idx](tower_output)
            outputs[name] = torch.sigmoid(task_output).squeeze(1)
