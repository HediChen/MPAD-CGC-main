...
class MMoe(nn.Module):
    def __init__(self,
        ...
        self.experts_dnn = nn.ModuleList([
            nn.Sequential(
                nn.Flatten(),
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        experts_output = [expert(x[:,:,:1008]) for expert in self.experts]
        for expert_dnn in self.experts_dnn:
            experts_output.append(expert_dnn(x[:,:,1008:])) 

        # Each task tower processes the merged expert outputs


        for idx, name in enumerate(self.labels_dict):
            gate = self.gate_dnn[idx](x[:,:,1008:])
