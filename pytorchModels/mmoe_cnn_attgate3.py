...
class CrossAttentionGate(nn.Module):
    def __init__(self, num_tasks: int, expert_dim: int):
        super(CrossAttentionGate, self).__init__()
        self.task_embeddings = nn.Embedding(num_tasks, expert_dim)
    ...
    def forward(self, experts: torch.Tensor, task_idx: int):
        """
        experts: (B, N, D), task_idx: int
        returns: (B, D)
        """
        B, N, D = experts.size()
        task_embed = self.task_embeddings(torch.tensor([task_idx], device=experts.device))
        task_embed = task_embed.expand(B, -1)  # (B, D)

        Q = self.q_proj(task_embed).unsqueeze(1)  # (B, 1, D)
...
class MMoe(nn.Module):
    def __init__(self,
        ...
        self.attn_module = CrossAttentionGate(num_tasks=len(labels_dict), expert_dim=output_dim)
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Expert outputs: (B, N, D)
        expert_outputs = [expert(x) for expert in self.experts]
        experts = torch.stack(expert_outputs, dim=1)

        # Task-wise attention and tower
        for idx, name in enumerate(self.labels_dict):
            attn_output = self.attn_module(experts, task_idx=idx)  # (B, D)
...
if __name__ == '__main__':
    ...
    dummy_input = torch.randn(4, 1, 1008)  # (B, C, L)
