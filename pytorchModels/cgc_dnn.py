...
class GateNetwork(nn.Module):
    def __init__(self, inputs_dim, dnn_hidden_units,
                 num_experts, shared_experts_dnn,
                 dnn_dropout=0.2,
                 dnn_activation='relu',
                 dnn_use_bn=False
                 ):
        ...
        self.gates = nn.Sequential(
                ...
                # nn.BatchNorm1d(128),
                nn.ReLU(),
                nn.Linear(128, num_experts + len(shared_experts_dnn))
                )

    def forward(self, inputs):
        # inputs: (batch_size, 1, 1008)
        experts_output = [expert(inputs[:,:1008]) for expert in self.shared_experts_dnn]
...
class CGC(nn.Module):
    ...
    def __init__(self,
        ...
        self.gate_network = nn.ModuleList([
            GateNetwork(inputs_dim, expert_hidden_units, num_task_experts, self.shared_experts_dnn)
