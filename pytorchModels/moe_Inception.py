...
class Moe(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> Dict[str, torch.Tensor]:
        outputs = OrderedDict()
        gate_weight = torch.softmax(self.gate(inputs[:,:,:1008]), dim=-1).unsqueeze(2)
        ...
        tower_input = torch.sum(experts_output * gate_weight, dim=1)
        for index, name in enumerate(self.labels_dict):

            tower_output = self.task_tower[index](tower_input)
