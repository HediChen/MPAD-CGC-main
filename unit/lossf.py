...
class FocalLoss_origianl(nn.Module):
    def __init__(self, alpha=0.25, gamma=2, reduction='mean'):
        super(FocalLoss_origianl, self).__init__()
        self.alpha = alpha
        self.gamma = gamma
...
class FocalLoss_Ada_flood(nn.Module):
    ...
    #     else:
    #         return focal_loss
    def forward(self, logits, targets, log_vars):
        """
        :param logits: Raw model outputs of shape (batch_size, ) or (batch_size, num_tasks)
        :param targets: Ground truth labels of shape (batch_size, ) or (batch_size, num_tasks)
        """
        ...
        if self.reduction == 'mean':
            ...
            loss  # 'none'
        return (loss-log_vars).abs()+log_vars
...
class CombinedLoss(nn.Module):
    def __init__(self, alpha=1, gamma=2, dice_weight=1.0, focal_weight=1.0, reduction='mean'):
        ...
        self.reduction = reduction
        self.focal_loss = FocalLoss(alpha=self.alpha, gamma=self.gamma, reduction=self.reduction)
