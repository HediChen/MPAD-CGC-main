...
def sum_parameters_by_layer(model: nn.Module, parent_name=""):
    # Iterate over all modules in the model
    for name, module in model.named_modules():
        total_params = 0
        # Iterate over parameters within the module
        for param in module.parameters():
            if param.requires_grad:
                total_params += param.numel()

        # Only print if the module has parameters
        if total_params > 0:
            # Print the module's total parameters, with indentation for submodules
            full_name = f"{parent_name}.{name}" if parent_name else name
            indent_level = full_name.count(".")
            indent = " " * (indent_level * 4)  # 4 spaces per level of indentation
            print(f"{indent}{full_name:50}: {total_params:10} parameters")